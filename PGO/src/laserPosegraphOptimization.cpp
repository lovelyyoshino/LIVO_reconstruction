...
void saveOdometryVerticesTUMformat(std::string _filename)
{
  ...
  for(const auto& _pose6d: keyframePoses) {
    ...
    Point3 t = pose.translation();
    Rot3 R = pose.rotation();
    Eigen::Quaterniond qua = Eigen::Quaterniond(pose.rotation().matrix());

    stream << std::fixed  <<  timeQue[count] << " "
           << t.x() << " " << t.y() << " " << t.z() << " "
...
gtsam::Pose3 doICPVirtualRelative( int _loop_kf_idx, int _curr_kf_idx )
{
    ...
    cureKeyframeCloudRegMsg.header.frame_id = "robot/odom";
...
void process_pg()
{
    while(1)
    {
		while ( !odometryBuf.empty() && !fullResBuf.empty() )
        {
            ...
            timeQue.push_back(timeLaser);       //  存储时间
        }
...
void visualizeLoopClosure()
{
    ...
    markerNode.header.frame_id = "robot/odom"; // camera_init
