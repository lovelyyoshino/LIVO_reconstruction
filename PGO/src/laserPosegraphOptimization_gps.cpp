            ...
            if( ! gtSAMgraphMade /* prior node */) {
                ...
                mtxPosegraph.lock();
                {
                    ...
                    if(hasGPSforThisKF) {
                        ...
                        odomGps.pose.pose.position.x = gps_xyz[0];
...
void pubMap(void)
{
    ...
    for (int node_idx=0; node_idx < recentIdxUpdated; node_idx++) {
        if(counter % SKIP_FRAMES == 0) {
            ROS_INFO("keyframePosesUpdated.x=%f,y=%f,z=%f",keyframePosesUpdated[node_idx].x,keyframePosesUpdated[node_idx].y,keyframePosesUpdated[node_idx].z);
