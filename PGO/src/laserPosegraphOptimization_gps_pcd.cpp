...
ros::Publisher pubMapAftPGO, pubOdomAftPGO, pubPathAftPGO ,pubOdomGps;
...
Eigen::Vector3d iSamInputGPS(double latitude, double longitude, double altitude)
{
        ...
        {
            zero_utm = xyz;     
            printf("\033[1;32m----> zero_utm==%.6f,%.6f,%.6f.\033[0m", (*zero_utm)[0], (*zero_utm)[1], (*zero_utm)[2]);
            // ROS_INFO("\033[1;32m----> zero_utm==%.6f,%.6f,%.6f.\033[0m",(*zero_utm)[0], (*zero_utm)[1], (*zero_utm)[2]);
        }
        xyz -= (*zero_utm);
...
void imu_cbk(const sensor_msgs::Imu::ConstPtr &msg_in)
{
    ...
    //curr_IMU = transformRotZ(curr_IMU,10.5); //由于imu安装角偏了3.5°
    get_imu = true;
}
void initNoises( void )
{
    ...
    double bigNoiseTolerentToXY = 5.0; // 1e9
    double gpsAltitudeNoiseScore = 5.0; // if height is misaligned after loop clsosing, use this value bigger
...
std::optional<gtsam::Pose3> doICPVirtualRelative( int _loop_kf_idx, int _curr_kf_idx )
{
    ...
    loopFindNearKeyframesCloud_jxx(cureKeyframeCloud, _curr_kf_idx, 0, _loop_kf_idx); // use same root of loop kf idx 
...
void process_pg()
{
    while(1)
    {
		while ( !odometryBuf.empty() && !fullResBuf.empty() && get_imu)
        {
            //
            ...
            if( !gpsOffsetInitialized ) {
                if(hasGPSforThisKF) { // if the very first frame 
                    ...
                    gpsLatitudeInitOffset = currGPS->latitude;
                    gpsLongitudeInitOffset = currGPS->longitude;
            ...
            if( ! gtSAMgraphMade /* prior node */) {
                ...
                mtxPosegraph.lock();
                {
                    ...
                    if(hasGPSforThisKF) {
                        ...
                        odomGps.header.frame_id = "robot/odom";
                        ...
                        odomGps.pose.pose.orientation.w = curr_IMU.orientation.w;
                        odomGps.pose.pose.position.x = gps_xyz_[0];
                        odomGps.pose.pose.position.y = gps_xyz_[1];
                        odomGps.pose.pose.position.z = gps_xyz_[2];
                        ...
                        gtsam::Point3 gpsConstraint(gps_xyz_[0], gps_xyz_[1], gps_xyz_[2]);
                        ...
                        }else{ //如果xy方差太大，只用z轴的约束
                           
                            mtxRecentPose.lock();
                            gtsam::Point3 gpsConstraint(recentOptimizedX, recentOptimizedY, gps_xyz_[2]); // in this example, only adjusting altitude (for x and y, very big noises are set) 
                            mtxRecentPose.unlock();
...
void visualizeLoopClosure()
{
        ...
        markerNode.header.frame_id = "robot/odom";
        ...
        markerEdge.header.frame_id = "robot/odom";
...
bool save_map_service(aloam_velodyne::SaveMapRequest &req, aloam_velodyne::SaveMapResponse &res)
{
        ...
        if (1)
        {
            cout << "=========================>saveing map ... at " << pgScansDirectory <<"Opt_map" << ".pcd"<<endl;
...
int main(int argc, char **argv)
{
    ...
    pubOdomGps = nh.advertise<nav_msgs::Odometry>("/GpsOdom", 5);
