...
dlio::OdomNode::OdomNode(ros::NodeHandle node_handle) : nh(node_handle) {
  ...
  this->lidar_sub = this->nh.subscribe("pointcloud", 2000,
  ...
  // this->img_sub_ = this->nh.subscribe("img", 10, &dlio::OdomNode::callbackImg, this,
//                                      ros::TransportHints().tcpNoDelay());
  ...
  this->crop.setNegative(true);     //   保留外点
...
void dlio::OdomNode::getParams() {
  ...
  ros::param::param<int>("~dlio/odom/keyframe/minNumPoints", this->keyframe_thresh_min_num_points_, 3000);
...
void dlio::OdomNode::publishPose(const ros::TimerEvent &e) {
  ...
  // only publish the pose when IMU is available
...
void dlio::OdomNode::deskewPointcloud() {
  ...
  extract_point_time = [&sweep_ref_time](boost::range::index_value<PointType&, long> pt)
    ...
    extract_point_time = [&sweep_ref_time](
      boost::range::index_value<PointType &, long> pt) { return pt.value().timestamp; };
  ...
  if ((this->scan_stamp - this->prev_scan_stamp) > 0.2) {
    ROS_FATAL("Bad time %lf, %lf", this->scan_stamp, this->prev_scan_stamp);
    first_valid_scan = false;
    return;
  }
  // IMU prior & deskewing for second scan onwards
  // 对于从第二次扫描开始的情况，根据前一次扫描的时间戳、姿态、位置和速度，使用IMU数据进行积分，得到一系列的帧（姿态矩阵）。
  // 如果积分成功，deskew_size将等于frames的大小，否则会发生同步问题。
  std::vector<Eigen::Matrix4f, Eigen::aligned_allocator<Eigen::Matrix4f>> frames;
  frames = this->integrateImu(this->prev_scan_stamp, this->lidarPose.q, this->lidarPose.p,
  ...
  if (frames.size() != timestamps.size()) {
    ROS_FATAL("Bad time sync between LiDAR and IMU! (frame size: %u, timestamps size: %u)", frames.size(),
              timestamps.size());

    this->T_prior = this->T;     //  更新T_prior为扫描时间戳对应的中值点处的估计位姿
    pcl::transformPointCloud(*deskewed_scan_, *deskewed_scan_, this->T_prior * this->extrinsics.baselink2lidar_T);
    this->deskewed_scan = deskewed_scan_;
    this->deskew_status = false;
    return;
...
void dlio::OdomNode::callbackPointCloud(const sensor_msgs::PointCloud2ConstPtr &pc) {
  ...
  if (pc->height * pc->width <= 100) {
    ROS_FATAL("[Odom] Low number of points (%u) to process for DLIO!", pc->height * pc->width);
    this->geo.first_opt_done = false;
    return;
  }
  ...
  if (this->current_scan->points.size() <= this->gicp_min_num_points_) {
    ROS_FATAL("[Odom] Low number of points (%ld) for GICP in the cloud!", this->current_scan->points.size());
    return;
  }
...
void dlio::OdomNode::callbackImg(const sensor_msgs::Image::ConstPtr &img) {
  // TODO
}
  ...
  if (!this->imu_calibrated) {
    ...
    if ((imu->header.stamp.toSec() - this->first_imu_stamp) < this->imu_calib_time_) {
      ...
      if (this->gravity_align_) {
        ...
        Eigen::Quaternionf grav_q = Eigen::Quaternionf::FromTwoVectors(grav_vec,
                                                                       Eigen::Vector3f(0., 0., this->gravity_));
...
void dlio::OdomNode::updateKeyframes() {
  ...
  // Note: must put in the end
  if (this->current_scan->points.size() < this->keyframe_thresh_min_num_points_) {
    newKeyframe = false;
    ROS_FATAL("[Odom] Low number of points (%ld) as a keyframe cloud!", this->current_scan->points.size());
  }
...
void dlio::OdomNode::buildKeyframesAndSubmap(State vehicle_state) {
  ...
  for (int i = this->num_processed_keyframes; i < this->keyframes.size(); i++) {
    ...
    this->publish_keyframe_thread = std::thread(&dlio::OdomNode::publishKeyframe, this, this->keyframes[i],
