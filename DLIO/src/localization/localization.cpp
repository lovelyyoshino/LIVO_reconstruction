...
,tf2_listener_(tf2_buffer_)
{
    ...
    pointcloud_map (pcd_path_);
    // 订阅lidar点云
    sensor_points_sub_ = nh_.subscribe("/avia/livox/lidar", 1, &GicpLocalizer::callback_pointcloud, this);
...
// GPS 坐标转换为 UTM，并设置为初始位姿
void GicpLocalizer::callback_init_pose_by_gps(const sensor_msgs::NavSatFix::ConstPtr& gps_msg) {
    if (gps_msg->status.status == sensor_msgs::NavSatStatus::STATUS_NO_FIX) {
        ROS_WARN("No GPS fix.");
        return;
    }

    // 使用 geodesy 转换 WGS84 坐标到 UTM 坐标
    ...
    // 创建 PoseWithCovarianceStamped 消息，使用 UTM 坐标
    geometry_msgs::PoseWithCovarianceStamped utm_pose_msg;
    utm_pose_msg.header.stamp = gps_msg->header.stamp;
    utm_pose_msg.header.frame_id = map_frame_;
    utm_pose_msg.pose.pose.position.x = utm_point.easting-utm_offset_.x();
    utm_pose_msg.pose.pose.position.y = utm_point.northing-utm_offset_.y();
    utm_pose_msg.pose.pose.position.z = utm_point.altitude-utm_offset_.z();

    // 为了简化，假设朝向北（沿 Y 轴正方向）
    tf2::Quaternion q;
    q.setRPY(0, 0, 0); // 没有旋转
    utm_pose_msg.pose.pose.orientation = tf2::toMsg(q);

    // 如果已经处于正确的坐标系中，则直接使用
    if (gps_msg->header.frame_id == map_frame_) {
        initial_pose_cov_msg_ = utm_pose_msg;
    } else {
        // 进行坐标系转换
        geometry_msgs::TransformStamped::Ptr TF_pose_to_map_ptr(new geometry_msgs::TransformStamped);
        try {
            // 假设 get_transform 函数是用来获取两个坐标系之间的变换
            if (get_transform(map_frame_, gps_msg->header.frame_id, TF_pose_to_map_ptr)) {
                tf2::doTransform(utm_pose_msg, initial_pose_cov_msg_, *TF_pose_to_map_ptr);
            } else {
                ROS_WARN("Failed to get transform from %s to %s", gps_msg->header.frame_id.c_str(), map_frame_.c_str());
            }
    ...
    init_pose = false;
    ROS_INFO("Initialized pose from GPS.");
}
...
void GicpLocalizer::callback_init_pose(
   const geometry_msgs::PoseWithCovarianceStamped::ConstPtr & initial_pose_msg_ptr){
    ...
    init_pose = false;
    ROS_INFO("Initialized pose from Topic.");
}

void GicpLocalizer::pointcloud_map(const std::string &path) {
        pcl::PointCloud<pcl::PointXYZ>::Ptr map_points_ptr(new pcl::PointCloud<pcl::PointXYZ>);

        // 从文件路径加载点云
        if (pcl::io::loadPCDFile<pcl::PointXYZ>(path, *map_points_ptr) == -1) {
            ROS_ERROR("Couldn't read file at path: %s", path.c_str());
            return;
        }
        ...
        voxelgrid_.setLeafSize(resolution_, resolution_, resolution_);
        voxelgrid_.filter(*filtered);

        // 设置GICP的输入目标
        vgicp_.setInputTarget(filtered);
...
void GicpLocalizer::init_params(){
    ...
    private_nh_.param("pcd_path", pcd_path_, std::string("pcd/DLIO.pcd"));
    double offsetX,offsetY,offsetZ;
    private_nh_.param("utm_offset_x", offsetX, 0.0);
    private_nh_.param("utm_offset_y", offsetY, 0.0);
    private_nh_.param("utm_offset_z", offsetZ, 0.0);
    utm_offset_ = Eigen::Vector3d(offsetX, offsetY, offsetZ);

    voxelgrid_.setLeafSize(leafsize_, leafsize_, leafsize_);
    vgicp_.setResolution(resolution_);
    vgicp_.setNumThreads(numThreads_);

    map_frame_ = "robot/odom";
